...
/*
THIS VERSION OF THE CODE WAS SLIGHTLY ADJUSTED (functions that start with tk_ were added) 
   in order to fit to my requirements !!
...
Class SQLiteDB {
   ...
   tk_getFullTable(qry, max_result:=0) {
      TB := ""
         this.GetTable(qry, TB, max_result ) 
      RRR := ""
         RRR := TB.tk_getDataAndHeader()
      return RRR
   }

   tk_getTableNoHeader(qry, max_result:=0) {
      TB := ""
         this.GetTable(qry, TB, max_result ) 
      RRR := ""
         RRR := TB.Rows
   ...
   ;;; START Extending standard Functionality fo the SQLiteDB Class

      ; ---------------------------------- TK ----
      tk_getDataAndHeader(){
         my_ret := []
         my_ret := this.Rows
         my_ret.InsertAt(1, this.ColumnNames)
         return my_ret
      }
      ...
      tk_getHeaderIndex(){
         my_ret := []
         Loop, this.ColumnNames {
               my_ret[this.ColumnNames[A_Index]] := A_Index
         }
         return my_ret
      }
   ...
   ; CONSTRUCTOR __New
   ; ===================================================================================================================
   __New(sqliteDLLPath:=".\SQLite.dll") {
      This.Base._SQLiteDLL := sqliteDLLPath
   ...
   loadToInMem(dbPathName)
   {
      This.openDB(":memory:","W", True)
      OutputDebug, % This._Handle
      targetHandle := This._Handle
      srcDB := new SQLiteDB(This._SQLiteDLL)
      srcDB.OpenDB(dbPathName,"W")
      srcHandle := srcDB._Handle
      ;z := srcDB.tk_get1stValue("Select count(*) from T_KNOWLEDGE")

      This._StrToUTF8("main", UTF8x)
      This._StrToUTF8("main", UTF8y)

      RC := DllCall("SQlite3.dll\sqlite3_backup_init", "Ptr", This._Handle, "Ptr", &UTF8x , "Ptr", srcDB._Handle, "Ptr", &UTF8y, "Cdecl Int")

      if( RC !=0)
      {
         RC2 := DllCall("SQlite3.dll\sqlite3_backup_step", "Ptr", RC, "Int", -1, "Cdecl Int")

         If (ErrorLevel) {
            This.ErrorMsg := "DLLCall sqlite3_backup_step failed!"
            This.ErrorCode := ErrorLevel
            Return False
         }
         If (RC2) {
         ...
         RC3 := DllCall("SQlite3.dll\sqlite3_backup_finish", "Ptr", RC, "Cdecl Int")

         If (ErrorLevel) {
            This.ErrorMsg := "DLLCall sqlite3_backup_finish failed!"
            This.ErrorCode := ErrorLevel
            Return False
         }
         If (RC3) {
      ...
      Else {
         OutputDebug, RC is 0         
         This.ErrorMsg := "DLLCall sqlite3_backup_init failed!"
         This.ErrorCode := ErrorLevel
         This.ErrorMsg := This._ErrMsg()
         This.ErrorCode := RC
         Return False
   ...
   storeTo(dbPathName)
   {
      ;check if there is a valid handle / TODO
      OutputDebug, % This._Handle

      targetDB := new SQLiteDB(This._SQLiteDLL)
      targetDB.OpenDB(dbPathName,"W")
      targetHandle := targetDB._Handle
      z := targetDB.tk_get1stValue("Select count(*) from T_KNOWLEDGE")
      This._StrToUTF8("main", UTF8x)
      This._StrToUTF8("main", UTF8y)

      RC := DllCall("SQlite3.dll\sqlite3_backup_init", "Ptr", targetDB._Handle, "Ptr", &UTF8x , "Ptr", This._Handle, "Ptr", &UTF8y, "Cdecl Int")
      if( RC !=0)
      {
         RC2 := DllCall("SQlite3.dll\sqlite3_backup_step", "Ptr", RC, "Int", -1, "Cdecl Int")
         If (ErrorLevel) {
